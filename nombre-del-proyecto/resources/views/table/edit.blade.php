    ...
    <div class="container mt-5 mb-5">
        <div class="bg-white shadow-md rounded-lg p-6 mx-auto w-full max-w-7xl">
                ...
                <form method="GET" action="{{ route('table.edit', ['table' => $table]) }}">
                    <div class="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        @foreach (array_keys((array) $data->first()) as $column)
                            @if ($column != 'id')
                                <div>
                                    <label for="{{ $column }}" class="block text-sm font-medium text-gray-700">{{ $column == 'owner_id' ? __('Owner') : $column }}</label>
                        ...
                        <div class="flex items-end space-x-2">
                            ...
                            <a href="{{ route('table.edit', ['table' => $table]) }}" class="px-4 py-2 bg-gray-500 text-white hover:bg-gray-700 transition duration-300 rounded">{{__('Reset')}}</a>
                        </div>
                    </div>
                </form><br>
                                ...
                                @foreach (array_keys((array) $data->first()) as $column)
                                    @if ($column != 'id')
                                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-800 uppercase tracking-wider border">
                                            {{ $column == 'owner_id' ? __('Owner') : $column }}
            ...
            <div class="mt-4 text-right">
                <a href="{{ url('/menu/gestionar') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                    {{__('Back')}}
                </a>
                <a href="{{ route('table.view', ['table' => $table]) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                    {{__('View Table')}}
                </a>
            </div> 
        </div>
